...
def greedy_iteration(A, PE, PN, score, ML_data):
    ...
    for i in range(num_nodes):
        for j in range(num_nodes):
            ...
            edge = (i,j)
            pot_A, pot_PE, pot_PN, did_change = add_remove_edge(pot_A, pot_PE, pot_PN, edge)
            if did_change:
    ...
    # Try all node color moves
    num_sup_nodes = sum([len(x) for x in PN])
    for num in range(num_sup_nodes):
        i = 0
        for part in PN:
            for supnode in part:
                if i == num:
        ...
        for new_color in other_colors:
            ...
            # Eval new CCDAG
            pot_A = A.copy()
            pot_PE = pickle.loads(pickle.dumps(PE, -1))
            pot_PN = pickle.loads(pickle.dumps(PN, -1))
            pot_score = score.copy()
            pot_ML_data = ML_data.copy()
            ...
            PN[new_color].remove(supnode_to_change)
        
        PN[old_color].append(supnode_to_change)
        PN = [x for x in PN if len(x)>0]



    # Try all edge color moves
    ...
    if num_edges == 0 or num_edges == 1:
        ...
        for num in range(num_edges):
            ...
            for i, part in enumerate(pot_PE):
                if edge_to_change in part:
                    old_color = i   
                else:
                    other_colors.append(i)

            if len(pot_PE[old_color]) != 1:
                other_colors.append(len(pot_PE))
                pot_PE.append([])

         
            pot_PE[old_color].remove(edge_to_change)


            for new_color in other_colors:
                pot_A = A.copy()
                pot_PN2 = pickle.loads(old_PN)
                pot_score = score.copy()
                pot_ML_data = ML_data.copy()

          
                pot_PE[new_color].append(edge_to_change)
                ...
                new_super_nodes = utils.get_supnodes(pot_PE, num_nodes)
                changed_super_nodes = []
                ...
                # New eval
                pot_score, *pot_ML_data = score_DAG_color_edit(pot_A, pot_PE, [sum(x,[]) for x in pot_PN2], pot_ML_data)
                if pot_score > best_score:
                    best_A = pot_A
                    best_PE = pickle.loads(pickle.dumps(pot_PE, -1))
                    best_PN = pot_PN2
                    best_score = pot_score
                    best_ML_data = pot_ML_data
                    best_move = "EC"

                pot_PE[new_color].remove(edge_to_change)
                

            pot_PE[old_color].append(edge_to_change)
            pot_PE = [x for x in pot_PE if len(x)>0]
            pot_PN = [x for x in pot_PN if len(x)>0]
...
def add_remove_edge(A, PE, PN, edge):
