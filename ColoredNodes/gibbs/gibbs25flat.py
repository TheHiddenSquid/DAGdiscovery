...
sys.path.append("../")
import utils
from gibbs import CausalMCMC_gibbs, MCMC_iteration, score_DAG_full
...
def main():
    ...
    for _ in range(num_iters):
        current_edge_array, current_partition, current_bic, current_ML_data, _  = MCMC_iteration(current_edge_array, current_partition, current_bic, current_ML_data)
        A = current_edge_array.astype("int")
        counts[dags.index(A.tobytes())] += 1
    ...
    plt.bar(range(25), [x/sum(counts) for x in counts], label="MCMC", color = bar_color)
