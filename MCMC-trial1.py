...
def main():
    ...
    for i in range(MCMC_iterations):
        ...
        old_edges = current_edge_array.copy()
        old_partition = current_partition.copy()

        new_edges, new_partition, q_quotient = update_DAG(old_edges, old_partition)

        new_bic = score_DAG(samples, new_edges, new_partition)
        old_bic = score_DAG(samples, old_edges, old_partition)

        if random.random() <= (new_bic / old_bic) * q_quotient: 
            current_edge_array = new_edges
            current_partition = new_partition
        else:
            current_edge_array = old_edges
            current_partition = old_partition

        if new_bic > best_bic:
            best_edge_array = new_edges.copy()
    ...
    plt.axes(ax3)
    color_map = generate_color_map(best_partition)
