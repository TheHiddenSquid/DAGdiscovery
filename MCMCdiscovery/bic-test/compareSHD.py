...
def main():
    ...
    for _ in range(num_tests):
        ...
        dags.append((real_partition, real_lambda_matrix, real_omega_matrix))


    for dag_id in range(num_tests):
        print(dag_id)
        for num_samples in [100, 500, 1000]:
            real_partition, real_lambda_matrix, real_omega_matrix = dags[dag_id]
            ...
            MCMC_edge_array, partition, bic = MCMCfuncs_new.CausalMCMC(samples, MCMC_iterations, start_from_GES = False)
            MCMC_error = utils.calc_SHD(dag_to_cpdag(real_edge_array), dag_to_cpdag(MCMC_edge_array))
            df.loc[-1] = [dag_id, num_samples, "new MCMC", MCMC_error]
            df.index = df.index + 1
            df = df.sort_index()


            MCMC_edge_array, partition, bic = MCMCfuncs_old.CausalMCMC(samples, MCMC_iterations, start_from_GES = False)
            MCMC_error = utils.calc_SHD(dag_to_cpdag(real_edge_array), dag_to_cpdag(MCMC_edge_array))
            df.loc[-1] = [dag_id, num_samples, "old MCMC", MCMC_error]
            df.index = df.index + 1
            df = df.sort_index()
    ...
    plt.title("Boxplot: p=4, sparse, tests=25, iters=50_000")
    ...
    # Creating plot
    sns.boxplot(data=df, x="sample size", y="SHD", hue="Algorithm")
    plt.title("Boxplot: p=4, dense, tests=25, iters=50_000")
    ...
    # # Creating plot
    # sns.boxplot(data=df, x="sample size", y="SHD", hue="Algorithm")
    # plt.title("Boxplot: p=12, sparse, tests=50, iters=100_000")
    ...
    # # Creating plot
    # sns.boxplot(data=df, x="sample size", y="SHD", hue="Algorithm")
    # plt.title("Boxplot: p=12, dense, tests=50, iters=100_000")
